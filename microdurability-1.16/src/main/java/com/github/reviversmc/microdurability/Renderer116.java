...
public class Renderer116 extends Renderer {
	...
	protected boolean hasMending(Object stack) {
		return EnchantmentHelper.getLevel(Enchantments.MENDING, (ItemStack) stack) > 0;
	}

	@Override
	protected void drawWarningTexture(Object texture, Object context, int x, int y, int u, int v, int width, int height) {
		mc.getTextureManager().bindTexture((Identifier) texture);
	...
	protected int getItemBarStep(Object stack) {
		float damage = ((ItemStack) stack).getDamage();
		float maxDamage = ((ItemStack) stack).getMaxDamage();
